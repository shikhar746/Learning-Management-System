...
import { MessageSquare, Plus, ArrowLeft, Loader2, AlertCircle, ShieldAlert } from "lucide-react"

export default function AdminWorkshopForumPage({ params }) {
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-0.5">
              Topic-based Q&A and asynchronous moderation for cohort participants.
            </p>
          </div>
        </div>
      ...
      {showCreateModal && (
        <form onSubmit={handleCreateTopic} className="bg-white border border-gray-200 rounded-xl p-5 space-y-4 max-w-xl">
          <h3 className="font-bold text-gray-900 text-sm">Create New Discussion Topic</h3>
          {error && <p className="text-xs text-red-600">{error}</p>}
          <div className="space-y-1">
            <Label htmlFor="title" className="text-xs">Topic Title</Label>
            <Input
              id="title"
              required
              placeholder="e.g. Project 1 Setup Errors & Q&A"
          ...
          <div className="space-y-1">
            ...
            <textarea
              id="description"
              rows={2}
              placeholder="Brief summary of what this thread covers..."
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          ...
          {topics.length === 0 ? (
            <div className="text-center py-8 text-gray-400 text-xs">
              No topics created yet. Click "+ New Discussion Topic" to get started.
            </div>
          ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-[10px] text-gray-400 mt-2">
                      <span>{t.createdBy?.name || "Instructor"}</span>
                      <span>{t._count?.posts ?? 0} posts</span>
                    </div>
                  </button>
                )
              })}
        ...
        {/* Right Column: Topic Discussion Thread Viewer */}
        <div className="lg:col-span-2">
          {selectedTopic ? (
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-16 text-center space-y-2 text-gray-400">
              ...
              <p className="text-sm font-semibold">Select a topic thread to view messages</p>
            </div>
          )}
        </div>
      </div>

...
import { ShieldCheck, Users, Crown, Key, BookOpen, Award, Search, Loader2, AlertCircle, ArrowRight } from "lucide-react"
    ...
    return (
      <div className="text-center py-16 space-y-4">
        ...
        <p className="text-xs text-gray-500 max-w-sm mx-auto">
          You must logged in with Super-Admin / OWNER role to access this control panel.
        </p>
        <Link href="/admin">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Total Workshops</span>
            <ShieldCheck className="w-4 h-4 text-purple-600" />
          </div>
          <p className="text-3xl font-extrabold text-gray-900">{stats.totalWorkshops}</p>
          <div className="flex items-center gap-2 text-[11px] text-gray-400">
            <span className="text-green-600 font-semibold">{stats.activeWorkshops} Active</span> ·{" "}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">System Users</span>
            <Users className="w-4 h-4 text-blue-600" />
          </div>
          <p className="text-3xl font-extrabold text-gray-900">{stats.totalUsers}</p>
          <div className="flex items-center gap-2 text-[11px] text-gray-400">
            <span className="text-blue-600 font-semibold">{stats.studentCount} Students</span> ·{" "}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Submissions</span>
            <BookOpen className="w-4 h-4 text-amber-600" />
          </div>
          <p className="text-3xl font-extrabold text-gray-900">{stats.totalSubmissions}</p>
          <div className="text-[11px] text-gray-400">
            <span className="text-amber-600 font-semibold">{stats.gradedSubmissions} Graded</span> ({Math.round((stats.gradedSubmissions / (stats.totalSubmissions || 1)) * 100)}%)
          </div>
        </div>

        <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Active Cohorts</span>
            <Key className="w-4 h-4 text-green-600" />
          </div>
          <p className="text-3xl font-extrabold text-gray-900">{stats.activeWorkshops}</p>
          <div className="text-[11px] text-gray-400">
              ...
              {filteredWorkshops.length === 0 ? (
                ...
                filteredWorkshops.map((w) => (
                  <tr key={w.id} className="hover:bg-gray-50/80 transition-colors">
                    <td className="p-3 font-semibold text-gray-900">{w.name}</td>
                    <td className="p-3 font-mono font-bold text-blue-700">{w.code}</td>
                    <td className="p-3 text-gray-600">{w.createdBy?.email || "System"}</td>
                    <td className="p-3 font-medium text-gray-800">{w.studentCount}</td>
                    <td className="p-3 font-medium text-gray-800">{w.adminCount}</td>
                    <td className="p-3 text-gray-600">{w.assignmentCount}</td>
                    <td className="p-3 text-amber-700 font-semibold">{w.certificateCount}</td>
                    <td className="p-3">
                      <span

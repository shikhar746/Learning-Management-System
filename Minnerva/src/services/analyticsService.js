  ...
  return getOrSetCache(cacheKey, 30, async () => {
    ...
    if (userRole === "OWNER") {
      ...
      const classAverages = assignmentsWithSubmissions.map((assign) => {
        ...
        const avg = scores.length > 0 ? Math.round(totalMarks / scores.length) : 0
        return {
          id: assign.id,
          title: assign.title,
    ...
    const perWorkshopResults = assignedWorkshops.map((workshop) => {
      ...
      const sumMarks = gradedScores.reduce((a, b) => a + b, 0)
      const averageScore = gradedScores.length > 0 ? Math.round(sumMarks / gradedScores.length) : 0
...
export const getStudentAnalytics = async (studentId) => {
  return getOrSetCache(`analytics:student:${studentId}`, 30, async () => {
    ...
    const gradedSubmissions = completedSubmissions
      .filter((s) => s.isGradePublished && s.totalScore !== null)
      .map((s) => ({
        ...s,
